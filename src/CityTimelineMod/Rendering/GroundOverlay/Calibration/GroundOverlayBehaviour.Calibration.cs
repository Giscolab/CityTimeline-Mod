...
namespace CityTimelineMod.Rendering
{
    internal sealed partial class GroundOverlayBehaviour
    {
        ...
        {
            ClearOverlayChildrenExcept(null);
        }

        private int ClearOverlayChildrenExcept(GameObject retainedChild)
        {
            ...
            for (var i = 0; i < transform.childCount; i++)
            {
                ...
                if (child != null && child != retainedChild)
                    children.Add(child);
            }
            ...
            foreach (var child in children)
            {
                ...
                {
                    childComplete = false;
                    Log.Error("GroundOverlay calibration: child could not be disabled before release. " + ex);
                }
        ...
        )
        {
            ...
            cube.name = name;
            cube.transform.SetParent(OverlayRenderParent, true);
