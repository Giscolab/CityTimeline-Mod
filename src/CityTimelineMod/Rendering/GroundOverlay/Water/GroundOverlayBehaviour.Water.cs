        ...
        )
        {
            ...
            foreach (var outline in outlines)
            {
                ...
                if (batch.Vertices.Count + vertices.Count >= maxVerticesPerMesh)
                    createdMeshObjects += OverlayMeshFlusher.FlushWaterAreaFillBatch(OverlayRenderParent, batch, LogVerboseOverlay);
            ...
            foreach (var line in lines)
            {
                ...
                for (var i = 0; i < line.Count - safeStride; i += safeStride)
                {
                    ...
                    if (batch.Vertices.Count + 8 >= maxVerticesPerMesh)
                        groupMeshObjects += OverlayMeshFlusher.FlushWaterBatch(OverlayRenderParent, namePrefix, batch, LogVerboseOverlay);
